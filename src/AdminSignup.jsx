...
export default function AdminSignup({ onBack }) {
  const [form, setForm] = useState({ username: '', email: '', department: '', password: '', confirm: '' });
  ...
  const handleSignup = async (e) => {
    ...
    setLoading(true);
    const result = await signup(form.username, form.email, form.password, 'admin', form.department || null);
    setLoading(false);
    if (result.success) {
      setSuccess('Administrator account created successfully.');
      setForm({ username: '', email: '', department: '', password: '', confirm: '' });
    } else {
  ...
  const fields = [
    { key: 'username',   label: 'Username',             type: 'text',     ph: 'Admin username' },
    { key: 'email',      label: 'Email Address',         type: 'email',    ph: 'admin@organisation.com' },
    { key: 'department', label: 'Department (Optional)', type: 'text',     ph: 'e.g. Operations, IT' },
        ...
        <div className="rounded-2xl p-8" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div className="mt-6 pt-5" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="text-center text-slate-600 text-xs" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Admin accounts carry full system permissions.
            </p>
          </div>
        </div>
      </div>
